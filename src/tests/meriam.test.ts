...
const meriamParser = MeriamParse;
test("API parsing", async () => {
  const response = await meriamParser(
    "points of view",
    new Response(meriamMockResponse)
  );
  expect(
...
test("API parsing, misspelled responses", async () => {
  const response = await meriamParser(
    "wand",
    new Response(meriamMockMisspeled)
  );
  expect(response.type == "error" ? response.errorMessage : undefined).toBe(
    APIErrorEnum.NoWord
  );
  const emptyResponse = await meriamParser("fasdnssobkjfd", new Response(`[]`));
  expect(
